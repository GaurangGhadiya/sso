const getIframeSSO = async (service_id, frame_type) => {
...
@media (max-width: 768px) {
  ...
  iframe {
    ...
    function closeIframe() {
        ...
        const refreshEvent = new Event('refreshParentEvent');
    ...
    window.addEventListener('refreshParentEvent', () => {
        // Perform the action to refresh the parent content
        location.reload();
    });
    ...
    if (frame_type === "register") {
        iframe.src = 'http://localhost:3000/ssonext/registration-iframe?service_id=' + service_id + "&is_iframe=true";

    }
    else if (frame_type === "forgot_password") {
        iframe.src = 'http://localhost:3000/ssonext/forgot-iframe?service_id=' + service_id + "&is_iframe=true";

    }
    else {
        iframe.src = 'http://localhost:3000/ssonext/login-iframe?service_id=' + service_id + "&is_iframe=true";
    }
    ...
    function resizeIframe() {
        ...
        if (iframe) {
            ...
            parent.postMessage(height, 'https://yourdomain.com'); // Replace with your domain
        }
    }

    // Call resizeIframe whenever the content changes or loads
    function contentChangeHandler() {
        resizeIframe();
    }

    // Add event listeners to handle content changes
    iframeContainer.addEventListener('load', contentChangeHandler);
    iframeContainer.addEventListener('change', contentChangeHandler);
