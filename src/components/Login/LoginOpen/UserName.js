		...
		if (
			...
			try {
				...
				if (userDetail) {
					...
					const response = await api.post("/user-login-open", { data: encryptBody(JSON.stringify(reqData)) });
			...
			if (decr) {
			  try {
				...
				data = json_data;
				  ...
				  let url = data.success_url;

				  const parameter = "token=" + data.tempToken;
				  ...
				  if (data.redirect_service_id) {
					  url += "&service_id=" + data.redirect_service_id;
				  }
