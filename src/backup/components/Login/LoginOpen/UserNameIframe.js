        ...
        if (responseData.user_check_api) {
          ...
          getUser(responseData.user_check_api, service_id, userName)
            .then((user) => {
              ...
              // }
              if (singleRowData) {
                Cookies.set("user_info", JSON.stringify(singleRowData), {
          ...
          //   setAlert({ open: true, type: false, message: "User Not Found" });
        }
      }
    } catch (error) {
      setLoading(false);
      ...
      try {
        ...
        if (userDetail) {
          if (otp.length === 0) {
            ...
            const { redirect, secondaryMapping, sso_id, PrimaryData } =
              response.data || {};
            ...
            if (response.status === 200) {
              ...
              if (secondaryMapping.length > 0) {
                ...
                if (service_id === "10000046") {
                  ...
                  Cookies.set("uid", sso_id, { expires: expirationDate });
                  Cookies.set("name", "", { expires: expirationDate });
                  ...
                  if(response.status==200) {
                    ...
                    if (decr) {
                      try {
                        ...
                        // const { secondaryMapping, redirect, sso_id } =
                        // response.data || {};
