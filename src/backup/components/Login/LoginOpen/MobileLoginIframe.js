...
const MobileLoginIframe = ({ service_id, handleChange, redirectForLogin }) => {
  ...
  const UserCheckMobile = async (mobile) => {
    if (mobile.length < 5) {
      return;
    }
    try {
      ...
      const responseURL = await api.post("/user-check-mobile", reqData);

      const { userCount, mobileUsers, user } = responseURL.data || {};
      ...
      if (userCount === 0) {
        ...
        if (mobileUsers.length > 1) {
          //   let array = [];
          //   for (let a = 0; a < mobileUsers.length; a++) {
          //     let object = {
          //       label: mobileUsers[a].userName,
          //       value: mobileUsers[a].id,
          //     };
          //     array.push(object);
          //   }
          //   setEmailList(array);
        } else if (mobileUsers.length === 1) {
          sendOtpNew("");
          const userData = await checkUser(mobile);

          // setselectedUsername(mobileUsers[0].id);
        }
      }
  ...
  const sendOtpNew = async (selected_user) => {
    ...
    if (mobile.length === 10) {
      ...
      try {
        const response = await api.post("/send-mobile-otp-new", reqData);
    ...
    if (Object.values(newError).every((value) => value !== true)) {
      try {
        ...
        if (emailList && emailList.length > 0) {
          ...
          const response = await api.post("/check-secondary-mapping", reqData);
      ...
      <Grid spacing={1} container>
        <Grid item xs={12}>
          ...
          <Space.Compact style={{ width: "100%" }}>
            ...
            />
            <AntdButton onClick={() => sendOtpNew()} type="primary">
              Get OTP
            </AntdButton>
          </Space.Compact>
        </Grid>
